...
defmodule Readiness do
  ...
  @supported %{
    ...
    # `is_nil` on a `T | nil` is `isNone`); `elem/2` and `tuple_size/1` read
    # the pair of a map entry; `div`/`rem` are non_neg_integer() only.
    # `is_binary/1` is not here: the model has no binaries, so it is never
    # decidable, not merely undecidable at some types.
    kernel_calls: [self: 0, not: 1, is_map_key: 2, map_size: 1, length: 1, hd: 1, tl: 1,
                   ...
                   is_boolean: 1, is_number: 1, is_float: 1, is_tuple: 1,
