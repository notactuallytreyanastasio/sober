...
#
#   elixir elixir/readiness.exs [--markdown] [--strict] [--no-translate] PATH...
#
...
# elixir/src). `--no-translate` skips step 1 (walker only).
#
...
defmodule Readiness do
  ...
  @supported %{
    ...
    builtin_types: [pid: 0, integer: 0, non_neg_integer: 0, boolean: 0],
    ...
    kernel_calls: [self: 0, not: 1, is_map_key: 2, map_size: 1],
    operators: [:+, :-, :++, :<=, :>=, :<, :>, :==, :!=, :and, :or],
    # `case` scrutinee: a variable, Map.get/2 or Map.fetch/2
    case_scrutinees: ["variable", "Map.get/2", "Map.fetch/2"],
  ...
  def main(argv) do
    {opts, paths} = Enum.split_with(argv, &String.starts_with?(&1, "--"))
    md? = "--markdown" in opts
    strict? = "--strict" in opts
    translate? = "--no-translate" not in opts
    if paths == [], do: die("usage: elixir elixir/readiness.exs [--markdown] [--strict] [--no-translate] PATH...")
    ...
    case Code.string_to_quoted(src, columns: false) do
      {:ok, ast} ->
        ...
        analyzed = for {name, segs, body, nested?, line} <- mods, cand = classify_module(body), cand != nil,
                       do: analyze_module(path, name, segs, body, nested?, cand, registered, reply?, line)
  ...
  defp run_translator(path, flags \\ [], tries \\ 0) do
    ...
    case {status, Regex.run(~r/^error: no registered name for (\S+):/, first)} do
      {0, _} -> {:ok, "translated" <> if(flags == [], do: "", else: " with " <> Enum.join(flags, " "))}
      {_, [_, name]} when tries < @pid_retries ->
        run_translator(path, flags ++ ["--pid", "#{name}=#{String.downcase(name)}"], tries + 1)
      _ -> {:error, if(first == "", do: "exit #{status}", else: String.replace_prefix(first, "error: ", ""))}
    end
  end
  ...
  defp analyze_module(path, name, segs, body, nested?, kind, registered, reply?, first_line) do
    ...
    fs =
      [
        if(length(segs) > 1, do: [finding(:blocker, first_line, "dotted module name", name)], else: []),
        if(nested?, do: [finding(:blocker, first_line, "nested defmodule", name)], else: []),
        for({:defstruct, m, _} <- body, do: finding(:blocker, ln(m, first_line), "defstruct")),
  ...
  defp check_types(mod, first_line) do
    ...
    [
      if(not Map.has_key?(types, :state), do: [finding(:blocker, first_line, "no @type state (untyped mode not available)")], else: []),
      if(msgs == [], do: [finding(:blocker, first_line, "no @type msg/cast/info/call (untyped mode not available)")], else: []),
  ...
  defp check_type(mod, t, line, where) do
    ...
    case t do
      ...
      {{:., _, [{:__aliases__, _, segs}, f]}, _, args} ->
        if {segs, f, length(args || [])} in @supported.remote_types,
          do: [], else: blk.("unsupported type #{Enum.join(segs, ".")}.#{f}()", str(t))
      a when is_atom(a) and a not in [nil, true, false] -> []
    ...
    pre_fs =
      for s <- pre do
        case s do
          ...
          _ -> [finding(:blocker, line_of(s, line), "statement in init/1", str(s))] ++ classify_stmt_kinds(mod, s, line_of(s, line))
        end
      end
    last_fs =
      case last do
  ...
  defp check_pattern(mod, p, line) do
    ...
    case p do
      ...
      {:=, _, _} -> [finding(:blocker, l, "pattern alias = on a non-map", str(p))]
      {:%{}, _, pairs} ->
        for {k, vp} <- pairs do
  ...
  defp check_body(ctx, b, line) do
    ...
    case b do
      ...
      {:case, _, [scrut, [do: arms]]} ->
        sfs =
          case scrut do
            ...
            _ -> [finding(:blocker, l, "case scrutinee is not a variable or Map.get/fetch", str(scrut)), check_expr(ctx.mod, scrut, l)]
          end
      ...
      {:try, _, [blocks]} ->
        ...
        [finding(:blocker, l, "try/" <> Enum.join(kinds, "/")), check_body(ctx, blocks[:do], l)]
  ...
  defp check_stmts(ctx, stmts, line) do
    ...
    case Enum.split_while(stmts, fn s -> not blocking_call?(s) end) do
      ...
      _ ->
        [for(s <- pre, do: check_stmt(ctx, s, line_of(s, line))), check_last(ctx, last, line_of(last, line), pre != [])]
    end
  end
  ...
  defp check_blocking(ctx, {:=, _, [lhs, {_, _, [target, m | _]}]}, l) do
    [
      ...
      if(ctx.mod.reply?, do: [], else: [finding(:blocker, l, "GenServer.call used but no @type reply in the file")]),
  ...
  defp check_stmt(ctx, s, l) do
    ...
    case s do
      ...
      {{:., _, [{:__aliases__, _, [:GenServer]}, :reply]}, _, [to, r]} ->
        [if(mod.reply?, do: [], else: [finding(:blocker, l, "GenServer.reply used but no @type reply in the file")]), check_expr(mod, to, l), check_expr(mod, r, l)]
      ...
      {:=, _, [lhs, rhs]} -> [finding(:blocker, l, "variable binding x = e", str(s)), check_pattern(mod, lhs, l), check_expr(mod, rhs, l)]
  ...
  defp check_last(ctx, last, l, pre?) do
    control? = match?({f, _, _} when f in [:if, :case, :cond, :with, :try, :unless, :receive], last)
    [
      if(pre? and control?, do: [finding(:blocker, l, "if/case after statements (only a whole-body if/case)", str(last))], else: []),
  ...
  defp check_expr(mod, e, line) do
    ...
    case e do
      ...
      {:@, _, [{name, _, _}]} -> [finding(:blocker, l, "module attribute @#{name} as a value", str(e))]
      {:^, _, _} -> blk.("pin ^x")
      {:&, _, _} -> blk.("capture &")
      {:fn, _, clauses} -> [blk.("anonymous fn"), for({:->, _, [_ps, b]} <- clauses, do: check_expr(mod, b, l))]
      {:<<>>, _, _} -> blk.("string interpolation / binary")
      {:sigil_s, _, _} -> blk.("string literal")
      {v, _, nil} when is_atom(v) -> []
      {:self, _, []} -> []
      {:%{}, _, [{:|, _, [m, updates]}]} -> [blk.("map update %{m | ..}"), check_expr(mod, m, l), for({_, v} <- updates, do: check_expr(mod, v, l))]
      {:%{}, _, pairs} ->
        cond do
          pairs == [] -> []
          Enum.all?(pairs, fn {k, _} -> is_atom(k) and k not in [nil, true, false] end) ->
            ...
            sev = if mod.atom_key_map?, do: :note, else: :blocker
            [finding(sev, l, "record-shaped map literal %{field: ..}", str(e)), for({_, v} <- pairs, do: check_expr(mod, v, l))]
          true -> for {k, v} <- pairs, do: [check_expr(mod, k, l), check_expr(mod, v, l)]
        end
      {:%, _, [_, {:%{}, _, [{:|, _, _}]}]} -> blk.("struct update %Mod{s | ..}")
      ...
      {{:., _, [{:__aliases__, _, segs}, f]}, _, args} when is_list(args) ->
        [finding(:blocker, l, "#{Enum.join(segs, ".")}.#{f}/#{length(args)}", str(e)) | Enum.flat_map(args, &check_expr(mod, &1, l))]
      ...
      {{:., _, [m, f]}, meta, []} when is_atom(f) ->
        if Keyword.get(meta, :no_parens, false) or match?({_, _, nil}, m),
          do: [finding(:blocker, l, "field access s.field", str(e))],
