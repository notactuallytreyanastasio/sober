  ...
  defp render(ctx, clauses) do
    ...
    sig =
      if ctx.effects do
        ...
        # without EXIT the codec is dead code but must still typecheck: use
        # the first nullary message
        dummy =
          Enum.find_value(ctx.msg_ctors, fn {tag, []} -> ".#{tag}"; _ -> nil end) ||
            fail("no nullary message to stand in for the unused EXIT codec; declare {:EXIT, pid(), term()} in @type msg")
        exit_line =
          if List.keymember?(ctx.msg_ctors, :EXIT, 0),
            do: "  exitMsg := fun p r => .EXIT p r\n",
            else: "  -- no module declares {:EXIT, ...}; nobody traps, so this codec is never used\n  exitMsg := fun _ _ => #{dummy}\n"
  ...
  defp plain_body(ctx, mod, env, stmts) do
    ...
    {state, tail} =
      case last do
        ...
        {f, _, args} when f in [:raise, :throw] and is_list(args) ->
          ctx.effects || fail("#{f} outside effects mode")
          st = env[{:alias, "__state__"}] || whole_state(env, ctor, fields)
          {st, [".exit .error"]}
