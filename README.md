...
| Log store | the count is the buffer's real length and the buffer never exceeds `max_entries` | 11,737, mutation-tested (counter bumped on both branches; counter maintained but buffer never appended) |
| Logs live | what the view shows stays a prefix of what it shows next (bounded check, 809 configurations, prepend mutant caught); that the view neither drops nor duplicates an entry is a *step* property the bounded check cannot express, proven instead as `stepConserves` with `not_dropping` and `not_duplicating` |
...
| `Leanactors/Examples/LogsLive.lean` | `EnsembleWeb.LogsLive`, the LiveView that shows what the store broadcasts: hand `beh`, `beh_eq_gen`, the prefix check and the prepend mutant, plus `stepConserves`/`not_dropping`/`not_duplicating` for what the bounded check cannot see; the docstring names `handle_event/3` as the transition the model does not carry |
