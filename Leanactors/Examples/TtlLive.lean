...
/-! ## Two more list facts: erasing an entry that is not the hit -/
...
theorem List.findIdx_eraseIdx_le {α : Type} {l : List α} {p : α → Bool} {i : Nat} {x : α}
  ...
  | cons a rest ih =>
    ...
    | zero =>
      simp only [List.getElem?_cons_zero, Option.some.injEq] at hx
      subst hx
      rw [List.eraseIdx_cons_zero, List.findIdx_cons_of_false hpx]
      exact Nat.le_succ _
    | succ j =>
      simp only [List.getElem?_cons_succ] at hx
      rw [List.eraseIdx_cons_succ]
...
/-! ## Generic `Sys` plumbing: timers only grow, except by firing -/
